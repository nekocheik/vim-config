 ...
 * 'string | number'
 */
export function stringifyTypeTree(typeTree, anonymousFunction = true) {
  if (typeTree.kind === "union") {
    const unionString = typeTree.types
      .map((t) => stringifyTypeTree(t))
      .join(" | ");
  ...
  if (typeTree.kind === "intersection") {
    const nonObjectTypeStrings = typeTree.types
      .filter((t) => t.kind !== "object")
      .map((t) => stringifyTypeTree(t));

    const objectTypes = typeTree.types.filter((t) => t.kind === "object");
    const objectTypesProperties = objectTypes.flatMap((t) => t.properties);
    const objectTypeExcessProperties = objectTypes.reduce(
      (acc, t) => acc + t.excessProperties,
      0
    );
    ...
    let propertiesString = typeTree.properties
      .map((p) => {
        ...
        if (p.type.kind === "union") {
          const hasUndefined = p.type.types.some(
            (t) => t.kind === "basic" && t.typeName === "undefined"
          );
          if (hasUndefined) {
            optional = "?";
          }
          // Remove undefined from union
          p.type.types = p.type.types.filter(
            (t) => t.kind !== "basic" || t.typeName !== "undefined"
          );
        }
  ...
  if (typeTree.kind === "array") {
    return `${typeTree.readonly ? "readonly " : ""}${stringifyTypeTree(
    ...
    const signatures = typeTree.signatures.map((s) => {
      ...
      const parametersString = parameters
        .map((p) => {
          ...
          return `${p.isRestParameter ? "..." : ""}${
            p.name
          }${optional}: ${stringifyTypeTree(p.type)}`;
        })
        .join(", ");

      return `(${parametersString})${returnTypeChar} ${stringifyTypeTree(
        returnType
      )}`;
    });
 ...
 */
export function getSyntaxKindDeclaration(syntaxKind, typeName) {
  switch (syntaxKind) {
...
/**
 * Prettifies the type string
 */
export function prettyPrintTypeString(typeStringInput, indentation = 2) {
  const typeString = typeStringInput
    .replace(
      /typeof import\(".*?node_modules\/(.*?)"\)/g,
      'typeof import("$1")'
    )
    .replace(/ } & { /g, " ");
...
/**
 * Sanitizes a string for cleaner output
 */
